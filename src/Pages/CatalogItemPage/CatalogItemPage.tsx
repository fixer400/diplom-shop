...
export default function CatalogPageItem(){
  ...
  function sendItemInCart(){
    if(selectedSize !== ''){
      dispatch(setProductCart({...product,size:selectedSize, amount:itemCount}))
          ...
          <div className="row">
            ...
            <div className="col-7">
              ...
              <div className="text-center">
                <p>Размеры в наличии: 
                  {product.sizes.map((data: {size:string, avalible:boolean}) => 
                    data.avalible && (
                      <span 
                        key={data.size} 
                        className={selectedSize === data.size ?"catalog-item-size selected" : "cataloga-item-size"}
